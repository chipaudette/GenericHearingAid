...
void setupTympanHardware(void) {
  ...
  //enable the output to begin the flow of audio
...
void setup() {
  ...
  Serial.println("GenericHearingAid: starting setup()...");

  // Audio connections require memory
  AudioMemory(10);      //allocate Int16 audio data blocks
  AudioMemory_F32(10,audio_settings);  //allocate Float32 audio data blocks
...
void loop() {
  ...
  printMemoryAndCPU(millis(),3000);  //print every 3000 msec
} //end loop()
...
void printMemoryAndCPU(unsigned long curTime_millis,unsigned long updatePeriod_millis) {
